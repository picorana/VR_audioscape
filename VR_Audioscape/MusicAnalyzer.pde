...
class MusicAnalyzer {
  ...
  float[] doTheMagic() {
    ...
    threshold = constrain(c[0]*totalBeat*.2 + map(constrain(totalGlobal, 0, 2), 0, 2, 4, 0), 5, 1000)*.8;
    //println(threshold);
    ...
    if (beat > threshold & beatCounter > 5) {
      ...
      float xPosition = random(-(tile_length*strips_width)*.5, tile_length*strips_width*.2);
